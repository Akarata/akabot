...
try {
		...
		const katalog = (teks) => {
             res = wibu.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 321, "message": teks, "footerText": "*_© A K A R A T A_*", "thumbnail": ofrply, "surface": 'CATALOG' }}, {quoted:ftrol})
             wibu.relayWAMessage(res)
        }
        const grupinv = (teks) => {
        	grup = wibu.prepareMessageFromContent(from, { "groupInviteMessage": { "groupJid": '6288213840883-1616169743@g.us', "inviteCode": 'https://chat.whatsapp.com/JI25LDtPW784GAZoRD9fW9', "groupName": `${NamaBot}`, "footerText": "*_© A K A R A T A_*", "jpegThumbnail": ofrply, "caption": teks}}, {quoted:finv})
...
│
└───「 \`\`\`${NamaBot}\`\`\` 」`
sendButLocation(from, `${menu}`, "*_© A K A R A T A_*", {jpegThumbnail:ofrply,name:""}, [{buttonId:`command`,buttonText:{displayText:'BUTTON MENU'},type:1},{buttonId:`owner`,buttonText:{displayText:'DEVELOPER'},type:1},{buttonId:`script`,buttonText:{displayText:'SOURCE CODE'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
}
break
case 'allmenu': 
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
stod = `${sender}`
       stst = await wibu.getStatus(`${sender.split('@')[0]}@c.us`)
       nama = '🦈 A K A R A T A‍ 🦈'
       ownerJid = "6289601738686@s.whatsapp.net"
				stst = stst.status == 401 ? '' : stst.status
			num = await fetchJson(`https://api.telnyx.com/anonymous/v2/number_lookup/${senderNumber}`, {method: 'get'})
menunya = `╭─❒ 「 Bot Info 」 ❒
🏴‍☠️ _Creator : *P  R  S* (CUMAN RECODE)
...
case 'cecan':
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
  sticWait(from)
anu = await fetchJson(`https://zeroyt7-api.herokuapp.com/api/asupan/cecan?apikey=${zerkey}`)
...
case 'rika':
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
  sticWait(from)
anu = await fetchJson(`https://zeroyt7-api.herokuapp.com/api/asupan/rikagusriani?apikey=${zerkey}`)
ano = await getBuffer(anu.result.url)
wibu.sendMessage(from, ano, video, { quoted: mek})
case 'hijaber':
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
  sticWait(from)
anu = await fetchJson(`https://zeroyt7-api.herokuapp.com/api/asupan/hijaber?apikey=${zerkey}`)
ano = await getBuffer(anu.result.url)
wibu.sendMessage(from, ano, video, { quoted: mek})
break
case 'ghea':
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
  sticWait(from)
anu = await fetchJson(`https://zeroyt7-api.herokuapp.com/api/asupan/ghea?apikey=${zerkey}`)
ano = await getBuffer(anu.result.url)
wibu.sendMessage(from, ano, video, { quoted: mek})
break
case 'bocil':
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
  sticWait(from)
anu = await fetchJson(`https://zeroyt7-api.herokuapp.com/api/asupan/bocil?apikey=${zerkey}`)
ano = await getBuffer(anu.result.url)
